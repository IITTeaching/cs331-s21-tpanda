    ...
    def insert(self,key):
        ...
        if key not in self:
            def join(tree, val):
                ...
                if tree.val > val:
                    return HBStree.INode(tree.val, join(tree.left, val), tree.right)
                elif tree.val < val:
                    return HBStree.INode(tree.val, tree.left, join(tree.right, val))
            if self.root_versions[-1]:
                self.root_versions.append(join(self.root_versions[-1], key))
            else:
