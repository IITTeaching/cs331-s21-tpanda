...
class AVLTree:
    ...
    def add(self, val):
        ...
        def addRecursive(n):
            if not n:
                ...
                n.left = addRecursive(n.left)
                AVLTree.rebalance(n)
                return n
            elif val > n.val:
                n.right = addRecursive(n.right)
                AVLTree.rebalance(n)
                return n

        recursive = addRecursive(self.root)
        self.root = recursive
        self.size += 1
        ### END SOLUTION
    ...
    def __delitem__(self, val):
        assert(val in self)
        ### BEGIN SOLUTION
        def delRecursive(n, k):
            if not n:
                return n
            elif k < n.val:
                n.left = delRecursive(n.left, k)
            elif k > n.val:
                n.right = delRecursive(n.right, k)
            else:
                if not n.left:
                    return n.right
                elif not n.right:
                ...
                while c.left:
                    c = c.left
                n.val = c.val
                n.right = delRecursive(n.right, c.val)
            AVLTree.rebalance(n)
            return n

        self.root = delRecursive(self.root, val)
        self.size -= 1
        ### END SOLUTION
